"""Given a single histomics image name, this file will download overlapping tiles and save them. 
...
parser.add_argument('--tile_save_path', 
    ...
    help="Path to save the .png tiles")
...
if __name__ == "__main__":
    ...
    tile_save_path = args.tile_save_path
    ...
    utils.verify_files([env_path, gtcodes_path, tile_save_path, label_save_path, spreadsheet_path])

    #Load roi codes and calculate tile size pre downsampling.  Will be downsampling to 1990x1990 at 2 micron_per_pixel so should be proportional size of original res
    roi_codes = pd.read_csv(spreadsheet_path, index_col="corresponding_wsi_number", engine='python')
    target_image_name = target_image.replace('.tiff', '')
    mpp_WSI = roi_codes.at[target_image_name, "mpp_wsi"]
    mpp_WSI = int(mpp_WSI[0])
    ...
    wsi_id = utils.get_sample_id(target_image, conn, COLLECTION_NAME, FOLDER_NAME)
    print(wsi_id)

    #create stuff to downsample the image by scale factor
        ...
        for col_idx, col in enumerate(left_coords):
            ...
            if on_bot:
                if on_right:
                    ...
                    tile = utils.grab_image_data(conn, wsi_id, mm_x, mm_y, col, col+mod_tile_size-cols_padding, row, row+mod_tile_size-rows_padding)
                    ...
                    tile = utils.grab_image_data(conn, wsi_id, mm_x, mm_y, col, col+mod_tile_size, row, row+mod_tile_size-rows_padding)
                    tile_np = np.array(tile)
                ...
                if np.sum(thumb_tile) == 0:
                    continue
                tile = utils.grab_image_data(conn, wsi_id, mm_x, mm_y, col, col+mod_tile_size-cols_padding, row, row+mod_tile_size)
                tile_np = np.array(tile)
                ...
                print(type(tile_annotation))
                tile_np = np.array(tile)
                tile_annotation_np = np.array(tile_annotation)
            # Save the tile and annotation
